...
CHAR* ReadDriverName(INT64 FLT_FILTERAddr) {
    ...
    DriverMemoryOperation((VOID*)&FilterNameLen, (VOID*)(FLT_FILTERAddr + Offset + 2), 2, MEMORY_READ);
    if (FilterNameLen == 0) {
        return NULL;
    }

    // Read filter name address
    DriverMemoryOperation((VOID*)&FilterNameAddr, (VOID*)(FLT_FILTERAddr + Offset + 8), 8, MEMORY_READ);
    if (!FilterNameAddr) {
        return NULL;
    }
    ...
    // Read filter name
    DriverMemoryOperation(FilterName, (VOID*)FilterNameAddr, FilterNameLen, MEMORY_READ);
