...
VOID PrintAndClearCallBack(INT64 PspNotifyRoutineAddress, CHAR* CallBackRegFunc) {
    ...
    for (UINT64 k = 0; k < MAX_CALLBACKS; k++) {
        ...
        // Read callback entry
        DriverMemoryOperation((VOID*)&buffer, (VOID*)(PspNotifyRoutineAddress + (k * 8)), 8, MEMORY_READ);
        if (!buffer) continue;
        ...
        DriverMemoryOperation((VOID*)&DriverCallBackFuncAddr, (VOID*)(tmpaddr + 8), 8, MEMORY_READ);
